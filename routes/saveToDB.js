...
const loadData = () => {try {const rawData = fs.readFileSync(dbFilePath, 'utf-8');return JSON.parse(rawData);
} catch (err){console.error('Error loading DB:', err.message);return null}};
...
router.get('/saveToDB/:id?', (req, res) => {
  ...
  // If no ID is provided, return dashboard stats
  const members = data;
  const now = new Date();
